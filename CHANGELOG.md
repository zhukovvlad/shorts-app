...
### Добавлено
- **🤖 Автоматическая санитизация промптов при ошибках модерации OpenAI**
  - Реализована система умного исправления отклонённых промптов через OpenAI Chat API
  - **3 попытки переписывания**: каждый отклонённый промпт автоматически переписывается через `gpt-4o-mini`
  - Промпт приводится в соответствие с политиками безопасности, сохраняя исходный смысл
  - Graceful degradation: пропуск изображения только после 3 неудачных попыток санитизации
  - Добавлено детальное логирование каждой попытки переписывания
  ...
  **Процесс обработки:**
  1. Обнаружена ошибка модерации → 2. Вызов OpenAI для переписывания промпта → 3. Повторная генерация изображения → 4. При неудаче: повтор до 3 раз → 5. Fallback: пропуск изображения
  
  **Улучшения качества:**
  - ✅ Большинство отклонённых промптов теперь успешно переписываются и проходят модерацию
  - ✅ Меньше пропущенных изображений в итоговом видео
  - ✅ Сохранение смысла оригинального запроса пользователя

## [1.7.1] - 2025-10-16

### Исправлено
- **🛡️ Обработка ошибок модерации контента OpenAI (Content Safety System)**
  - Реализован graceful degradation для отклоненных изображений
  - Видео теперь создается, если хотя бы одно изображение успешно сгенерировано
  - Улучшено логирование: WARN для ошибок модерации, ERROR для технических ошибок
  - Добавлена детальная статистика: успешные/отклоненные/всего изображений
  - Каждое изображение обрабатывается изолированно (независимо от других)
  - Документация: `docs/CONTENT_MODERATION_HANDLING.md`
  
  **До исправления:**
  - ❌ Одно отклоненное изображение роняло всё видео
  - ❌ Пользователь терял результат, даже если 4/5 изображений были успешны
  
  **После исправления:**
  - ✅ Отклоненные изображения пропускаются с предупреждением
  - ✅ Видео создается с оставшимися успешными изображениями
  - ✅ Понятное логирование причины отклонения

## [1.7.0] - 2025-10-16

### Добавлено
- **💰 Дифференцированная система оплаты кредитами за модели генерации изображений**
  - Введены коэффициенты стоимости для различных моделей генерации изображений
  - Базовая стоимость (1 кредит) умножается на коэффициент модели для расчета итоговой цены
  - Округление вверх для предотвращения дробных кредитов
  - Минимальная стоимость: 1 кредит
  
  **Коэффициенты моделей:**
  - `FLUX Schnell`: 1.0x (1 кредит)
  - `DALL-E 2`: 1.0x (1 кредит)
  - `Stable Diffusion XL`: 1.2x (2 кредита)
  - `Ideogram V3 Turbo`: 1.5x (2 кредита)
  - `FLUX Dev`: 1.5x (2 кредита)
  - `FLUX Pro`: 2.0x (2 кредита)
  - `DALL-E 3`: 2.0x (2 кредита)
  - `DALL-E 3 HD`: 2.5x (3 кредита)
  
  **Новые функции:**
  - `getModelCoefficient(modelIdOrName)` - получить коэффициент для модели
  - `computeModelCost(modelIdOrName, baseCost = 1)` - вычислить стоимость в кредитах
  - Файлы: `lib/imageModels.ts`

- **UI: Отображение стоимости моделей при выборе**
  - В селекторе моделей теперь отображается стоимость каждой модели в кредитах
  - Динамическое вычисление стоимости для выбранной модели
  - Проверка достаточности кредитов перед созданием видео с учетом стоимости модели
  - Файлы: `app/new/CreateProject.tsx`

- **Backend: Атомарное списание кредитов с учетом стоимости модели**
  - Серверная функция `createVideo` теперь вычисляет стоимость модели перед транзакцией
  - **Валидация входящей модели:** проверка существования `imageModel` в `IMAGE_MODELS`
  - **Единый `resolvedModelId`:** используется один валидированный ID модели для всех операций
  - **Логирование неизвестных моделей:** warning при попытке использовать несуществующую модель
  - Атомарная проверка и списание кредитов: `WHERE credits >= cost` вместо `> 0`
  - **Улучшен `decreaseCredits`** с атомарной защитой от отрицательного баланса:
    - Валидация `amount` на конечность: `Number.isFinite()` защита от NaN/Infinity перед `Math.floor()`
    - Санитизация `amount` через `Math.floor()` после проверки конечности
    - Валидация `userId`: `.trim()` и проверка на whitespace-only строки
    - Использование `trimmedUserId` во всех операциях для консистентности
    - Использование `updateMany` с условием `credits >= amt`
    - Проверка результата и выброс ошибки при недостаточном балансе
    - Явный тип возврата `Promise<void>`
  - Защита от состояний гонки при конкурентном доступе
  - Улучшенные информативные сообщения об ошибках:
    - "userId is required and cannot be whitespace-only"
    - "amount must be a finite number"
    - "Insufficient credits or user not found"
  - Файлы: `app/actions/create.ts`, `app/lib/decreaseCredits.ts`

- **Testing: Комплексное тестирование системы коэффициентов и списания кредитов**
  - 43 новых unit-теста для функций `getModelCoefficient` и `computeModelCost`
  - 25 новых unit-тестов для функции `decreaseCredits` (атомарность, валидация, санитизация)
    - Тестирование успешного списания с различными amount
    - Валидация параметров (empty userId, whitespace-only userId, trim userId)
    - Защита от невалидных чисел (NaN, Infinity, -Infinity)
    - Защита от отрицательного баланса
    - Санитизация дробных значений
    - Атомарность операций
  - Тестирование по ID и имени модели
  - Тестирование fallback-поведения для неизвестных моделей
  - Тестирование округления и минимальной стоимости
  - Тестирование с различными базовыми стоимостями
  - Интеграционные тесты по всем моделям в `IMAGE_MODELS`
  - Файлы: `lib/imageModels-coefficients.spec.ts`, `app/lib/decreaseCredits.spec.ts`

### Изменено
- **Breaking Change: Формат списания кредитов**
  - Функция `createVideo` теперь списывает переменное количество кредитов в зависимости от модели
  - Старая логика (всегда 1 кредит) заменена на динамическую с коэффициентами
  - Обратная совместимость: модели без коэффициента используют 1.0x (1 кредит)

### Технические детали
- **Алгоритм вычисления стоимости:**
  ```typescript
  cost = Math.max(1, Math.ceil(baseCost * coefficient))
  ```
  - Округление вверх (`Math.ceil`) гарантирует целое число кредитов
  - Минимальная стоимость 1 кредит предотвращает бесплатное использование
  - Базовая стоимость (по умолчанию 1) может быть переопределена для будущего масштабирования
...
- **Транзакционная безопасность:**
  ...
  ```
  - Атомарная проверка и списание в одной операции
  - Невозможность создать отрицательный баланс
  - Защита от race conditions при параллельных запросах

- **Валидация модели:**
  ```typescript
  // Проверяем существование модели в IMAGE_MODELS
  ...
  if (modelExists) {
    ...
    logger.warn('Unknown image model provided, falling back to default', {
      userId, providedModel: imageModel, defaultModel: getDefaultModel().id
    });
    resolvedModelId = getDefaultModel().id;
  }
  ```
  - Единый `resolvedModelId` используется для вычисления стоимости и сохранения в Redis
  - Warning логируется при попытке использовать несуществующую модель
...
- **Атомарная защита decreaseCredits:**
  ```typescript
  export const decreaseCredits = async (userId: string, amount = 1): Promise<void> => {
    const amt = Math.floor(amount); // Санитизация
  ...
  ```
  - Защита от отрицательного баланса через `updateMany` с условием
  - Санитизация amount через `Math.floor()` для предотвращения дробных кредитов
  - Валидация userId перед операцией
  - Проверка результата операции и информативные ошибки
  - Покрыто 19 unit-тестами (валидация, атомарность, санитизация)

- **UI/UX:**
  - Информация о стоимости отображается в реальном времени при выборе модели
  - Пользователь видит стоимость до создания видео
  - Блокировка создания видео при недостаточном балансе с учетом стоимости модели
  - Плавное UX: проверка на клиенте + серверная валидация

### Nitpick Улучшения (дополнительные оптимизации)
- **Оптимизация производительности:**
  - Устранено дублирование вызова `computeModelCost()` в SelectItem map (было 2 раза, стало 1)
  - Вычисление стоимости теперь происходит один раз на итерацию

- **UX улучшения:**
  - Диалог недостаточных кредитов теперь показывает конкретные цифры:
    - "You need 2 credits for FLUX Pro. You have 1." вместо общего сообщения
  - Пользователь видит требуемое и доступное количество кредитов

- **Hardening и надежность:**
  - Добавлен `MODEL_COEFFICIENTS_BY_ID` для работы по стабильным ID (снижена связность с UI-строками)
  - `Object.freeze()` для предотвращения случайных мутаций коэффициентов в runtime
  - `.trim()` в `getModelCoefficient()` для защиты от пробелов в начале/конце
  - Приоритет поиска: сначала по ID (надежно), затем по имени (совместимость)

- **Документация и планирование:**
  - TODO-комментарий для будущего сохранения стоимости в БД (поле `creditsCharged` в Video)
  - Преимущества: аудит, поддержка refund, аналитика по моделям

- **Дополнительное тестирование:**
  - +3 теста для whitespace robustness (trailing/leading spaces)
  - Документирована case sensitivity системы (by design)

- **Финальные улучшения безопасности и качества (Nitpicks #38-42):**
  - `Object.prototype.hasOwnProperty.call()` для безопасного поиска свойств в `MODEL_COEFFICIENTS_BY_ID` и `MODEL_COEFFICIENTS`
  - Защита от prototype pollution и корректная работа с falsy значениями (0, false)
  - Удален хрупкий тест проверки отсутствия `prisma.user.update` (-1 тест, покрытие сохранено)
  - Исправлена грамматика в русской документации (добавлена запятая)
  - Верифицировано наличие тестов для NaN/Infinity и whitespace userId (реализовано в Замечании #11)

### Статистика
- **Измененные файлы:** 5 файлов (основная функциональность) + 5 файлов (nitpicks #32-37) + 2 файла (замечание #11) + 3 файла (nitpicks #38-42)
- **Новые файлы:** 2 тестовых файла
- **Добавленные тесты:** 70 тестов (43 коэффициенты + 24 decreaseCredits + 3 robustness)
- **Удалено тестов:** 1 хрупкий тест
- **Общее количество тестов:** 234 (все прошли успешно)
- **TypeScript:** без ошибок компиляции
- **ESLint:** 0 ошибок, 75 предупреждений (все существующие)
- **Code Review:** 11 замечаний исправлено, 42 nitpicks реализовано = 53 улучшения ✅

### Миграция
Для существующих пользователей изменения не требуют миграции:
- Старые записи о видео не затрагиваются
- Существующий баланс кредитов сохраняется
- Все модели поддерживают обратную совместимость
- При отсутствии модели в коэффициентах используется 1.0x

## [1.6.9] - 2025-10-15

### Добавлено
- **UI: Textarea Component**
  - Добавлен компонент `Textarea` из shadcn/ui для многострочного ввода
  - Полная стилизация в соответствии с дизайн-системой проекта
  - Поддержка темной темы и всех accessibility атрибутов
  - Файлы: `components/ui/textarea.tsx`

### Улучшено
- **Mobile: Оптимизация страницы видео для мобильных устройств**
  - Адаптивные отступы: уменьшенные padding на мобильных (`px-3 sm:px-4`, `pt-2 sm:pt-3`)
  - Вертикальное расположение action-кнопок на мобильных (flex-col → flex-row)
  - Компактные Badge с меньшими размерами на мобильных (`text-xs sm:text-sm`, `h-6 sm:h-auto`)
  - Уменьшенные иконки на мобильных (`h-3 w-3 sm:h-3.5 sm:w-3.5`)
  - Меньший border-radius на мобильных для видео (`rounded-xl sm:rounded-2xl`)
  - Оптимизированные gap между элементами (`gap-4 sm:gap-6`)
  - Flex-wrap для Badge, чтобы они переносились на новую строку
  - Файлы: `app/videos/[videoId]/page.tsx`

- **UX: Многострочное отображение промпта**
  - Заменен однострочный `Input` на многострочный `Textarea`
  - Текст промпта теперь отображается полностью без обрезания
  - Автоматический перенос длинных промптов на несколько строк
  - Начальная высота в 3 строки для комфортного чтения
  - Файлы: `app/videos/[videoId]/page.tsx`
...
### Добавлено
- **UI: Галерея изображений на странице видео**
  - Создан новый компонент `ImageGallery` для отображения всех изображений, использованных в видео
  - Адаптивная сетка миниатюр (2-5 колонок в зависимости от размера экрана)
  - Модальное окно (Dialog) для просмотра изображений в полном размере при клике
  - **Навигация между изображениями:**
    - Клавиатурная навигация: стрелки ← → для переключения
    - Видимые кнопки-стрелки (ChevronLeft/ChevronRight) по бокам изображения
    - Кнопки условно рендерятся (скрыты на первом/последнем изображении)
    - Полупрозрачный фон с backdrop-blur эффектом
  - Красивые hover-эффекты с анимациями и тенями
  - Индикатор "Scene N" при наведении на миниатюру
  - Оверлей с номером сцены в модальном окне
  - Accessibility: `DialogTitle` с `sr-only` для screen readers
  - Файлы: `app/components/ImageGallery.tsx`, `app/videos/[videoId]/page.tsx`

### Улучшено
- **Image Gallery: Оптимизация производительности и UX**
  - **Thumbnails:** Включена Next.js Image Optimization для миниатюр
    - Автоматическая конвертация в WebP/AVIF форматы
    - Адаптивные размеры для разных устройств
    - Значительное снижение потребления трафика
  - **Full-size images:** Используют `unoptimized` prop для показа оригинального качества
  - **Обработка ошибок загрузки:** Fallback UI при таймаутах или ошибках загрузки S3 изображений
  - Подсказки пользователю в модальном окне
  - Файлы: `app/components/ImageGallery.tsx`

### Исправлено
- **Next.js Config: Удалена избыточная настройка и исправлены комментарии**
  - Удалена строка `unoptimized: false` (это default поведение, не требует явного указания)
  - Исправлен misleading комментарий о "таймауте" (unoptimized не управляет таймаутами)
  - Добавлен корректный комментарий о контроле оптимизации на уровне компонентов
  - Устранено несоответствие между глобальной настройкой и component-level пропсами
  - **Важно**: Next.js по умолчанию конвертирует изображения в AVIF/WebP (это стандартное поведение Image Optimization API). В `ImageGallery` используется `unoptimized` для полноразмерных изображений из-за S3 timeouts.
  - Файлы: `next.config.ts`

- **Accessibility: Исправлена ошибка Radix UI Dialog**
  - Добавлен обязательный `DialogTitle` компонент в `ImageGallery`
  - Title скрыт визуально через `sr-only`, но доступен для screen readers
  - Устранена console error: "DialogContent requires a DialogTitle"
  - Файлы: `app/components/ImageGallery.tsx`
...
- **API: Повышена безопасность и надежность cache revalidation**
  ...
  - **Утилита для воркера (`lib/revalidate.ts`):**
    - **Улучшена терминология:** параметр переименован `maxRetries` → `maxAttempts` для ясности
    - **Умная retry логика:** 
      ...
      - **410 Gone** добавлен в non-retryable для избегания бесполезных попыток
      - Поддержка заголовка `Retry-After` для 429 Too Many Requests
      - Парсинг `Retry-After` в формате секунд и HTTP-date
      - **Защита от NaN:** Fallback на экспоненциальную задержку если дата невалидная
