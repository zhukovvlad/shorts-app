...
- **API: Повышена безопасность и надежность cache revalidation**
  - **Allowlist тегов:** Только разрешенные теги могут быть инвалидированы (предотвращает злоупотребление)
  - **Retry логика:** Автоматический повтор до 2 раз при транзиентных ошибках (timeout, сеть)
  - **Экспоненциальная задержка:** 1s, 2s между попытками для стабильности
  - **Timeout:** 5 секунд на запрос (предотвращает зависание воркера)
  - **Умный retry:** Не повторяет при клиентских ошибках (400/401)
  - **Документирование:** Добавлен комментарий о необходимости rate limiting на уровне nginx
